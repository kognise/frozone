...
const tree = (directory, callback, root = directory.length) => {
  for (let item of fs.readdirSync(directory)) {
    const path = `${directory}/${item}`

    if (fs.statSync(`${directory}/${item}`).isDirectory()) {
      tree(`${directory}/${item}`, callback, root)
    } else {
      callback(path.slice(root + 1))
    }
  }
}
...
const getCustomDocument = (dist) => {
  if (!fs.existsSync(`${dist}/pages/_document.js`)) {
    const { code } = babel.transformFileSync('default/_document.js', babelConfig)
    fs.outputFileSync(`${dist}/pages/_document.js`, code)
  }

  const requirePath = `./${dist}/pages/_document`
...
module.exports.transformAll = (src, dist) => {
  tree(src, (file) => {
    ...
    delete require.cache[require.resolve(`./${dist}/${file}`)]
  })
}
...
module.exports.getPages = (dist) => {
  ...
  tree(`${dist}/pages/`, (file) => {
    if (file === '_document.js') return
    const PageInner = esRequire(`./${dist}/pages/${file}`)
